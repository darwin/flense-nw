...
(defn pair->lines [[left right]]
  (if (> (+ (chars left) 1 (chars right)) *line-length*)
    (vec (concat (->lines left) (->lines right)))
    [(concat (->tokens left) (->tokens right))]))
...
(defn header+body->lines [form headc]
  (let [[inits rests] (split-at (inc headc) (:children form))
        init-line (apply concat (opener form) (map ->tokens inits))
        rest-lines (mapcat ->lines rests)]
    (close-off `[~init-line ~@rest-lines] (closer form))))
...
(defn letlike->lines [form]
  (let [[head bvec & body] (:children form)]
    (if (= (:type bvec) :vec)
      (let [[init & more] (map->lines bvec)
            init-line (concat (opener form) (->tokens head) init)
            bvec-lines more
            body-lines (mapcat ->lines body)]
